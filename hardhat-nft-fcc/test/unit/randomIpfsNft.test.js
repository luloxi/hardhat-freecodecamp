      ...
      describe("fulfillRandomWords", () => {
        it("mints NFT after random number is returned", async function () {
          await new Promise(async (resolve, reject) => {
            // Set a listener for NftMinted event
            randomIpfsNft.once("NftMinted", async () => {
              try {
                // Get the tokenURI of the first emitted NFT
                const tokenUri = await randomIpfsNft.tokenURI("0")
                // and get the tokenCounter number
                const tokenCounter = await randomIpfsNft.getTokenCounter()
                // tokenURI should contain an IPFS address
                assert.equal(tokenUri.toString().includes("ipfs://"), true)
                // and tokenCount should be 1
                assert.equal(tokenCounter.toString(), "1")
                // If all true, resolve the promise
                resolve()
              } catch (e) {
                console.log(e)
                reject(e)
            ...
            // Request the Nft that will emit the NftMinted event
            try {
              const fee = await randomIpfsNft.getMintFee()
              ...
              // From the requestNftReceipt, get the requestId and address of the minting contract
